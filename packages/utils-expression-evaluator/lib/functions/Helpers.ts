...
import { alignGeometryCRS} from '../util/EPSGDefs';
...
addInvalidHandling = false,
  ...
  public onFeature1Literal1<T extends ISerializable>(
    op: (expressionEvaluator: IInternalEvaluator) =>
    (geom: GJ.Feature, lit: E.Literal<T>) => StringLiteral | DoubleLiteral | IntegerLiteral | BooleanLiteral,
      addInvalidHandling = true,
  ): Builder {
    return this.set(
      [ 'literal', 'literal' ],
      // eslint-disable-next-line max-len
      expressionEvaluator => ([ geom, lit ]: E.Literal<T>[]) => op(expressionEvaluator)(parseGeometryFeature(geom)[0], lit),
      addInvalidHandling,
    );
  }
...
addInvalidHandling = true,
  ...
  public geometryTestNormalizedCRS<T extends ISerializable>(
    ...
    return this
      .set(
        [ C.TypeURL.WKT_LITERAL, C.TypeURL.WKT_LITERAL ],
        expressionEvaluator => ([ left, right ]: E.Literal<T>[]): BooleanLiteral => {
